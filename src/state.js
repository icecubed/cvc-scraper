...
class State {
  constructor(id, name, districts) {
    ...
    this.districts = districts.map(
      (d) => new District(d.district_id, d.district_name, this)
    );
  }
  ...
  async getCVCLocations() {
      ...
      async (district) => {
        ...
        await pMap(
          ...
          async (cvc) => {
            ...
            console.log(
              `Processing ${this.name} -> ${district.name} -> ${cvc.name}`
            );
            let row = {
    ...
    return {
      ...
      rows: Object.values(rows).sort((a, b) =>
        `${a.District}-${a.CVC}`.localeCompare(`${b.District}-${b.CVC}`)
      ),
      header: header,
    };
  }
  ...
  async getEighteenData() {
    ...
    const header = ["District", "Block Name", "Center", "PIN"];
    const dates = new Set();
        ...
        for (const center of centers) {
          for (const session of center.sessions) {
            if (session.min_age_limit < 45) {
              console.log(
                `Found ${JSON.stringify(center)} with session ${JSON.stringify(
                  session
                )}`
              );
    ...
    return {
      ...
      rows: Object.values(rows).sort((a, b) =>
        `${a.District}-${a["Block Name"]}-${a.Center}`.localeCompare(
          `${b.District}-${b["Block Name"]} - ${b.Center}}`
        )
      ),
    };
  }
...
async function getAllStates() {
  ...
  return await pMap(states, async ({ state_id, state_name }) => {
    ...
    return new State(
      state_id,
      state_name,
      (await axios.get(distUrl)).data.districts
    );
  });
}
