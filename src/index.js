...
const EIGHTEEN_SHEET = "1pnaU6yjJfRkwWl1QA3PAAatcaiLp1Qkjw0VkvtT6vlQ";
...
async function updateCVCVaccinesData(states, days) {
  ...
  pMap(
    states,
    async (state) => {
      await state.publishData(SHEET_ID, CREDS, async (state) =>
        state.getCVCData(dates)
      );
    },
    { concurrency: 1 }
  );
...
async function updateCVCLocations(states) {
  pMap(
    states,
    async (state) => {
      await state.publishData(
        LOCATION_SHEET_ID,
        CREDS,
        async (state) => await state.getCVCLocations()
      );
    },
    { concurrency: 4 }
  );
...
async function eighteen(states) {
  for (const state of states) {
    await state.publishData(
      EIGHTEEN_SHEET,
      CREDS,
      async (state) => await state.getEighteenData()
    );
  }
}
...
async function main() {
  program
    .option("--state <states...>", "States to scrape")
    .option("--all-states", "Scrape all states")
    .option(
